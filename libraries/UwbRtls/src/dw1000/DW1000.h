...
//private:
	...
	/* Arduino interrupt handler (SPI-heavy — must run in TASK context only).
	 * The pin ISR is onIrqEdge(), which just sets _irqTriggered; call pollIrq()
	 * frequently from loop()/wait loops to service events. Never attach
	 * handleInterrupt() to the pin directly on ESP32 (SPI-in-ISR deadlock). */
	static void handleInterrupt();
	static void pollIrq();
	static void onIrqEdge();
	static volatile bool _irqTriggered;
