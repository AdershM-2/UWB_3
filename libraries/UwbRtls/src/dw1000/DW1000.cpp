...
void DW1000Class::begin(uint8_t irq, uint8_t rst) {
	...
	// Deferred interrupt handling (ESP32 IWDT fix, 2026-07-02): the pin ISR only
	// sets a flag; the SPI-heavy handleInterrupt() runs in task context via
	// pollIrq(). Running SPI inside the ISR deadlocks on ESP32 whenever the IRQ
	// fires mid-SPI-transaction in the main loop (spinlock held by the
	// interrupted task) -> "Interrupt wdt timeout on CPU1" panic. Seen in the
	// field as both tags crash-looping under dual-tag traffic.
	attachInterrupt(digitalPinToInterrupt(_irq), DW1000Class::onIrqEdge, RISING);
}
...
volatile bool DW1000Class::_irqTriggered = false;

// Minimal hardware ISR: flag only, no SPI, IRAM-safe (no flash-cache dependency).
void IRAM_ATTR DW1000Class::onIrqEdge() {
	_irqTriggered = true;
}

// Task-context IRQ service: call frequently from loop()/wait loops. Runs the
// SPI-heavy handler only when the ISR flagged an event. The DW1000 IRQ line
// stays HIGH until its status bits are cleared, so re-check the pin level
// after handling to catch events that arrived while we were busy (a RISING
// edge can be missed if the line never dropped).
void DW1000Class::pollIrq() {
	if(!_irqTriggered) return;
	_irqTriggered = false;
	handleInterrupt();
	if(digitalRead(_irq) == HIGH) _irqTriggered = true;
}
